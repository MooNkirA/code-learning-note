...
线程池是 JDK 1.5 后的新特性。不再需要开发者自定义容器，直接使用官方提供的即可。一个用来创建和管理线程对象的容器。线程池做的工作主要是管理线程组，控制运行的线程的数量及其运行状态。
...
#### 2.2.3. ThreadPoolExecutor 拒绝(饱和)策略
...
## 4. Executors 创建的 5 种线程池

Java 里面线程池的顶级接口是 `Executor`，但是严格意义上讲 `Executor` 并不是一个线程池，而只是一个执行线程的工具。真正的线程池接口是 `ExecutorService`。

以下是 `Executors` 工具类的静态方法，用于创建不同类型的线程池。
