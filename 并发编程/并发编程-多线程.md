...
一个父进程退出，而它的一个或多个子进程还在运行，那么这些子进程将成为孤儿进程。孤儿进程将被 init 进程（进程 ID 为 1）所收养，并由 init 进程对它们完成状态收集工作。因为孤儿进程会被 init 进程收养，所以孤儿进程不会对系统造成危害。
...
**并行（parallel）**，是同一时刻处理（doing）多件事情的能力，并且多个不同的任务同时在执行。这个需要多核 CPU 才能完成，每个核（core）都可以调度运行线程，在微观上就能同时执行多条指令，不同的程序被放到不同的处理器上运行，这个是物理上的多个进程同时进行。此时线程才是真正的并行（parallel）。

两者区别总结：<font color=red>**并发是交替执行，并行是同时执行。**</font>
...
> Notes: 主要涉及 `Thread` 类的 `interrupt`、`isInterrupted`、`interrupted` 等方法的应用，API 说明详见前面章节。
...
- 优先级的作用：优先级高的线程，被 CPU 执行到的概率大一些。
...
方式1：使用线程的 stop 方法强行终止。（不推荐，并且该方法已经过时）

方式2：使用共享变量的方式。设置一个被多个执行相同任务的线程的共享变量，用来作为是否停止的信号，通知停止线程的执行。通常会使用 `volatile` 关键字修饰此共享变量，保持线程的可见性。
...
- RedSpider 社区整理 - 深入浅出Java多线程。[官网版](http://concurrent.redspider.group/RedSpider.html) | [GithubBook 版](https://redspider.gitbook.io/concurrent/)
