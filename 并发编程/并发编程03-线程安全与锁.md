...
### 3.5. Lock 和 synchronized

#### 3.5.1. synchronized 和 ReentrantLock 的区别
...
#### 3.5.2. synchronized 和 ReentrantLock 的选择

从性能上考虑，如果有很多个线程同时抢夺资源时，`Lock` 接口的效率是要高于 `synchronized`，如果抢夺资源不激烈时，两个性能差不多。

### 3.6. tryLock、lock 和 lockInterruptibly 的区别
...
### 5.7. 使用 synchronized 实现 volatile 的效果
