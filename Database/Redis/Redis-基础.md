...
Redis 6.0 引入多线程，是用**多线程来处理数据的读写和协议解析，但是 Redis 执行命令还是单线程**的。
...
- 排查慢查询：使用 `slowlog get{n}` 命令可以获取最近的 n 条慢查询命令。
