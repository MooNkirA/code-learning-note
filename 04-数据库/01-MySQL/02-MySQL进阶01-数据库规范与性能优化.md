...
*注意：char和varchar跟字符编码也有密切的联系，比如latin1占用1个字节，gbk占用2个字节，utf8占用3个字节*
...
##### 3.1.15.5. Using index condition
...
##### 3.1.15.7. Using join buffer (Block Nested Loop)
...
##### 3.1.15.9. Using intersect(...)、Using union(...)和 Using sort_union(...)
...
##### 3.1.15.10. Zero limit

当 LIMIT 子句的参数为 0 时，表示不打算从表中读出任何记录，将会提示该额外信息。

##### 3.1.15.11. Using filesort(文件排序)
...
![](images/20210502192829293_28804.png)
...
##### 3.1.15.13. Start temporary, End temporary
...
##### 3.1.15.14. select tables optimized away
...
![](images/20210502152725921_14694.png)

![](images/20210502152732133_7010.png)

显然，order_no 列上的索引就比 order_status 列上的索引的选择性就要好，离散性更高。

#### 4.2.3. 前缀索引

有时需要索引很长的字符列，这会让索引变得大且慢。一种解决方案是**模拟哈希索引**

模拟哈希索引的做法：一个表中a字段很长，想把它作为一个索引，我们可以增加一个 a_hash 字段来存储 a 的哈希值，然后在 a_hash 上建立索引，相对于之前的索引速度会有明显提升，一个是对完整的 a做索引，而后者则是用整数哈希值做索引，显然数字的比较比字符串的匹配要高效得多。
...
于是有改进的方案，可以索引开始的部分字符，这样可以大大节约索引空间，从而提高索引效率。一般情况下需要保证某个列前缀的选择性也是足够高的，以满足查询性能。（尤其对于 BLOB、TEXT 或者很长的 VARCHAR 类型的列，应该使用前缀索引，因为 MySQL 不允许索引这些列的完整长度）。
...
建立了联合索引列，如果在查询语句中无法包含全部联合索引中的列时，但要遵守最左前缀法则。指的是查询从索引的最左前列开始并且不跳过索引中的列。
...
```sql
explain select * from order_exp_cut where insert_time='2021-03-22 18:34:55' and order_status=0 and expire_time>'2021-03-22 18:23:57' and expire_time<'2021-03-22 18:35:00' ;
```

![](images/20210502221718758_15006.png)

中间有范围查询会导致后面的列全部失效，无法充分利用这个联合索引

![](images/20210502221640102_3141.png)
...
![](images/20210502222430886_23270.png)

#### 4.3.7. Null/Not Null 对索引的影响
...
5. 通过以下语句能够看到执行过程中线程的每个状态和消耗的时间
...
##### 1.1.2.2. 情况二：主键列可以是范围匹配

分析与 Intersection 合并同理

##### 1.1.2.3. 情况三：使用 Intersection 索引合并的搜索条件
...
#### 1.1.4. 联合索引替代 Intersection 索引合并
...
#### 1.2.3. 基于块的嵌套循环连接（Block Nested-Loop Join）
...
#### 2.1.2. 常量传递（constant_propagation）
...
#### 2.1.3. 移除没用的条件（trivial_condition_removal）
...
### 2.3. 子查询优化（TODO mark: 待补充）

#### 2.3.1. 子查询的执行方式

#### 2.3.2. 标量子查询、行子查询的执行方式

#### 2.3.3. MySQL 对 IN 子查询的优化
