...
> redo log（重做日志）、undo log（撤销日志）主要用于实现事务，此部分内容详见[《MySQL数据库-事务》笔记](/数据库/MySQL/MySQL-04-事务)
...
binlog 二进制日志记录了对 MySQL 数据库所有执行过的修改操作，若操作本身没有导致数据库发生变化，该操作可能也会写入二进制文件。但是不包括 `select` 和 `show` 这类操作（因为这些操作对数据本身不会进行修改）。如果 MySQL 服务意外停止，可通过二进制日志文件排查，用户操作或表结构操作，从而来恢复数据库数据。
