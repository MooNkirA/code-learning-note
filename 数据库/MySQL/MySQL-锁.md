...
以上示例的处理方式效率太低。因此 InnoDB 提出了一种意向锁（英文名：Intention Locks），就是用于解决该问题。引入意向锁之后，客户端一，在执行 DML 操作时，会对涉及的行加行锁，同时也会对该表加上意向锁。
