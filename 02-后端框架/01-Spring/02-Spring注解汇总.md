...
此部分内容示例暂时未整理，先详见[Spring源码分析02-IOC.md](#/02-后端框架/03-Spring/03-Spring源码分析02-IOC)的示例

#### 2.5.5. 实现原理分析

以上`@Import`注解的实现原理，与`ImportSelector`、`ImportBeanDefinitionRegistrar`、`DeferredImportSelector`接口的调用逻辑，此部分内容详见[Spring源码分析02-IOC.md](#/02-后端框架/03-Spring/03-Spring源码分析02-IOC)
