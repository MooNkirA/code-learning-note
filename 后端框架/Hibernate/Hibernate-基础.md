...
- `hibernate.temp.use_jdbc_metadata_defaults`：当出现 `creation as createClob() method threw error` 异常时，
...
- 打开并且启动事务（增删改操作必须）。继承自 `org.hibernate.SharedSessionContract`。<font color=purple>**注：在同一个 session 对象中，如果多次 `beginTransaction()`，并且在事务没有提交的情况下，都是同一个事务对象。**</font>
...
### 5.2. Eclipse 插件 ERMaster 根据数据库表生成 JavaBean 代码
...
### 5.3. Hibernate 的 session 跟 servlet 的 session 的 区别
