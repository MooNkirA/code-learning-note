...
##### 2.5.6.1. 新生代为什么要分 Eden 和两个 Survivor 区域
...
#### 7.5.3. Parallel Scavenge 收集器（多线程复制算法、高效）
...
#### 7.5.4. Serial Old 收集器（单线程标记整理算法）
...
#### 7.5.5. Parallel Old 收集器（多线程标记整理算法）
...
### 7.7. finalize() 方法的调用与析构函数(finalization)
...
1. JVM 遇到一条新建对象的指令时，首先会检查这个指令的参数是否能在常量池中定位到一个类的符号引用，并检查引用代表的类是否已被加载、解析和初始化。如果没有，则执行类加载。
2. 通过检查通过后虚拟机将为新生对象分配内存。一种办法“指针碰撞”、一种办法“空闲列表”，最终常用的办法“本地线程缓冲分配(TLAB)”
3. 完成内存分配后，虚拟机将除对象头外的对象内存空间初始化为0。
4. 进行对象头必要的设置。包括哈希码、GC 信息、锁信息、对象所属类的类元信息等。
...
### 9.3. Java 对象内存分配是如何保证线程安全的

- 方法一：采用 CAS 机制，配合失败重试的方式保证更新操作的原子性。该方式效率低。
- 方法二：每个线程在 Java 堆中预先分配一小块内存，然后再给对象分配内存的时候，直接在自己这块"私有"内存中分配。一般采用这种策略。
...
### 10.2. 如何获取 Java 程序使用的内存？堆使用的百分比？
