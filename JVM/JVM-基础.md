...
#### 7.3.5. MinorGC、MajorGC、FullGC 的触发时机与区别

- **MinorGC**：回收新生代，因为新生代对象存活时间很短，因此 MinorGC 会频繁执行，执行的速度一般也会比较快。
- **MajorGC** 指的是老年代的 GC，出现 MajorGC 一般经常伴有 MinorGC。
- **FullGC**：回收老年代和新生代，老年代的对象存活时间长，因此 FullGC 很少执行，执行速度会比 MinorGC 慢很多。

##### 7.3.5.1. Full GC 的触发条件
...
### 7.6. finalize() 方法的调用与析构函数(finalization)
