...
> Notes: <font color=red>**不能以返回值类型的不同作为重载函数的区分标准**</font>
...
- 抽象类中的方法可以有方法体，就是能实现方法的具体功能；但是接口中的方法不行（Java 8 之后可以定义`default`关键字修饰的非抽象方法）。
- 抽象类中的成员变量可以是各种访问类型的；而接口中的成员变量只能是 `public static final` 类型的。
...
浅拷贝：拷贝对象和原始对象的引用类型，引用同一个对象内存地址。
...
### 10.3. 两个对象的 hashCode 相同，则 equals 是否也一定为 true？
...
#### 11.5.3. StringBuilder 与 String 相互转换方法
