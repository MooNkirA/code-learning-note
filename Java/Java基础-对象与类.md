...
5. 多继承
    ...
    - 一个类可以实现多个接口
6. 成员变量、静态变量
    - 抽象类可以定义成员变量与静态变量（包括常量），并且抽象类中的静态成员变量可以是任意访问类型。
    - 接口不可以定义成员变量，但是可以定义常量。接口的字段（变量）默认都是 `public static final` 修饰的类型。
7. 普通方法
    - 抽象类可以有普通方法
    - 接口在 JDK1.8 之前只能有抽象方法；JDK1.8 之后可以定义默认方法。如果有普通方法，普通方法也是可以由现实类重写。
8. 静态方法
    - 抽象类中可以包含静态方法
    - 接口在 JDK1.8 之后可以定义静态方法。
9. 抽象类与接口之间的继承性
