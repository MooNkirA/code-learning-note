...
Java 语言是强类型语言，对于每一种数据都定义了明确的具体的数据类型，在内存中分配了不同大小的内存空间。Java 语言数据类型分为两种：**基本数据类型（内置）**和**引用数据类型（对象）**。

![Java数据类型.xmind](images/161594818255839.png)

#### 3.1.1. 基本数据类型

| 基本类型 | 位数 | 大小（字节） |     默认值      |   包装类   |
| :-----: | :--: | :--------: | :------------: | :-------: |
|  byte   |  8   |     1      |    (byte)0     |   Byte    |
|  short  |  16  |     2      |    (short)0    |   Short   |
|   int   |  32  |     4      |       0        |  Integer  |
|  long   |  64  |     8      |       0L       |   Long    |
|  float  |  32  |     4      |      0.0f      |   Float   |
| double  |  64  |     8      |      0.0d      |  Double   |
| boolean |  1   |     -      |     false      |  Boolean  |
|  char   |  16  |     2      | `\u0000`(null) | Character |
...
- public：任意包下任意类都可以访问。可以修饰类、接口、变量、方法。
- protected：任意包下任意子类或同包下的任意类可以访问，可以修饰变量、方法。<font color=red>**注意：不能修饰类（外部类）**</font>。
- 默认(包权限，不使用任何修饰符)：同包下的任意类都可以访问。可以修饰在类、接口、变量、方法。
- private：可以修饰变量、方法，在同一类内可见。<font color=red>**注意：不能修饰类（外部类）**</font>

**修饰符权限列表图**：

|        可见性         | public | protected | 空的（default） | private |
| -------------------- | :----: | :-------: | :------------: | :-----: |
| 同一类中              |   ✅   |    ✅     |       ✅       |   ✅    |
| 同一包中（子类与无关类） |   ✅   |    ✅     |       ✅       |   ❌    |
| 不同包的子类           |   ✅   |    ✅     |       ❌       |   ❌    |
